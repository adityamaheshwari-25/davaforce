...
const isBenchMovementQuestion = (message: string) =>
  /\b(bench movement|capacity outlook|trend|over time|timeline|next weeks|30\/60\/90)\b/i.test(message) ||
  (/\b(30 days|60 days|90 days)\b/i.test(message) && /\b(bench|capacity|movement|outlook|trend)\b/i.test(message));
...
const fallbackMessage = (userMessage: string, details: WorkspaceChatDetails) => {
  ...
  const approvalDecision = details.json.approvalDecision as NonNullable<WorkforceRouterOutput["approvalDecision"]> | undefined;
  ...
  if (approvalDecision) {
    const blockerCount = approvalDecision.ewaSummary.blockers.length;
    const totalRequests = approvalDecision.ewaSummary.totalRequests;
    const conditions = approvalDecision.approvalPackage.conditions.slice(0, 3).join(" ");

    return markdownBullets([
      approvalDecision.recommendationSummary,
      `${approvalDecision.decisionState}. Ready for approval: ${approvalDecision.readyForApproval ? "yes" : "no"}.`,
      `EWA review found ${totalRequests} request(s) and ${blockerCount} blocker(s).`,
      conditions || "No approval conditions were returned for the selected option.",
      "No approval, booking, or EWA update was executed.",
    ]);
  }
